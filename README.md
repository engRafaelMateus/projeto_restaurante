...
[Funcionalidades](#funcionalidades) · [Executar localmente](#executar-localmente) · [Decisões técnicas](#decisões-técnicas) · [Testes](#testes) · [Autor](#autor)

![Painel do caixa com mesas e detalhes da comanda](docs/capturas/03-caixa-comanda.png)
...
1. O garçom abre uma mesa e seleciona os produtos, adicionais e observações.
2. O servidor valida o pedido, calcula os preços e grava os itens.
3. O caixa acompanha as comandas com atualização a cada 10 segundos.
4. O operador registra o pagamento manual e encerra a comanda.
5. A mesa é liberada e o consumo permanece no histórico.

## Funcionalidades

| Área | O que está implementado |
|---|---|
| **Cardápio** | Produtos por categoria, disponibilidade, imagens opcionais e adicionais; consulta pública do catálogo. |
| **Garçom** | Abertura de mesa, rascunho de pedido, quantidades, observações e envio de novas rodadas. |
| **Pizzas** | Seleção de dois sabores e regra configurável de preço: maior valor ou média. |
| **Caixa** | Visão das mesas, detalhes da comanda, atualização periódica e cancelamento de itens com motivo. |
| **Pagamento** | Registro manual de dinheiro, Pix, débito ou crédito; cálculo de troco em dinheiro e encerramento da comanda. |
| **Histórico** | Consulta por período, mesa e situação, com valores e responsáveis registrados. |
| **Administração** | Cadastros de produtos, adicionais, mesas e usuários pelo Django Admin. |
| **Acesso** | Perfis de garçom, caixa e administração, com permissões verificadas no servidor. |
...
![Pagamento de uma comanda](docs/capturas/04-caixa-pagamento.png)

**Histórico de atendimentos**

![Histórico de comandas](docs/capturas/05-caixa-historico.png)
...
| Problema | Solução no projeto | Código de referência |
|---|---|---|
| O navegador pode enviar um preço adulterado | A entrada recebe IDs, quantidades e escolhas; o servidor consulta o catálogo e calcula os valores. | [Formulários](apps/atendimento/formularios.py) e [serviços](apps/atendimento/servicos.py) |
| Uma tentativa de envio pode ser repetida após falha de rede | Cada envio possui UUID com restrição única. A repetição da mesma referência reutiliza o registro anterior. | [Serviços](apps/atendimento/servicos.py) |
| Duas comandas abertas para a mesma mesa causam inconsistência | Uma restrição condicional no banco limita a mesa a uma comanda aberta. | [Modelos](apps/atendimento/models.py) |
| Alterar o catálogo não pode mudar contas anteriores | Os itens preservam descrição, preço unitário e adicionais praticados na venda. | [Modelos](apps/atendimento/models.py) |
| Valores monetários exigem uma regra consistente | Cálculos com `Decimal`, duas casas decimais e arredondamento definido em um módulo próprio. | [Valores](apps/atendimento/valores.py) |
| Pagamento e encerramento precisam permanecer consistentes | As duas operações ficam na mesma transação; o pagamento possui relação um para um com a comanda. | [Serviços](apps/atendimento/servicos.py) |
| Ocultar um botão não impede acesso ao endpoint | As operações verificam o perfil no servidor e usam proteção CSRF nas mutações. | [API](apps/atendimento/api.py) e [perfis](apps/atendimento/perfis.py) |
...
|---|---|
| `apps.cardapio` | Categorias, produtos, adicionais e disponibilidade. |
| `apps.atendimento` | Mesas, comandas, envios, itens e pagamentos. |
| `apps.clientes` | Página pública do estabelecimento. |
| `models.py` | Entidades, relacionamentos e restrições de persistência. |
| `formularios.py` | Validação das entradas. |
| `servicos.py` | Operações de negócio e transações. |
| `api.py` e `views.py` | Endpoints JSON, autorização e apresentação das telas. |
| `static/js/` | Interações do navegador e comunicação HTTP. |
...
Mais detalhes em [Arquitetura e decisões](docs/ARQUITETURA.md).
...
As dependências de execução estão fixadas em [requirements.txt](requirements.txt). As ferramentas de desenvolvimento estão em [requirements-dev.txt](requirements-dev.txt).
...
| Usuário | Perfil | Tela principal |
|---|---|---|
| `garcom` e `garcom2` | Garçom | `/atendimento/comanda/` |
| `caixa` | Caixa | `/atendimento/caixa/` |
| `gerente` | Administração | `/admin/` |
...
1. Entre como `caixa` em uma janela normal e como `garcom` em uma janela anônima.
2. No garçom, abra a mesa 5 e adicione produtos com uma observação.
3. Envie a rodada e acompanhe a atualização na tela do caixa.
4. Adicione outra rodada; recarregue a página para conferir a persistência dos itens enviados.
5. No caixa, registre um pagamento em dinheiro e confira o troco.
6. Consulte o histórico e abra uma nova comanda na mesma mesa.
...
O [workflow de verificação](.github/workflows/ci.yml) está configurado para checagens do Django, migrações, carga de demonstração, arquivos estáticos, testes e lint. O resultado de uma execução remota deve ser consultado na aba Actions; a existência do workflow não significa que o CI já passou.
...
1. Validar concorrência em PostgreSQL antes de oferecer suporte a esse banco.
2. Implementar pagamentos divididos e descontos com regras explícitas.
3. Evoluir relatórios de vendas e integração com a cozinha.
...
[GitHub](https://github.com/engRafaelMateus) · [LinkedIn](https://www.linkedin.com/in/rafael-lopes-mateus-85331475/) · [E-mail](mailto:eng.rafaelmateus@gmail.com)
