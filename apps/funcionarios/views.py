...
def normalize_itens(raw):
    ...
    if isinstance(raw, str):
        try:
            ...
            return [{'nome': raw, 'qtd': 1, 'preco': 0, 'subtotal': 0}]
    if isinstance(raw, list):
        for entry in raw:
            nome = entry.get('item') or entry.get('nome') or ''
            qtd = int(entry.get('quantidade') or entry.get('qtd') or 1)
            preco = float(entry.get('preco') or 0)
            itens.append({'nome': nome, 'qtd': qtd, 'preco': preco, 'subtotal': round(qtd * preco, 2)})
    return itens
...
class ComandaView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ctx = super().get_context_data(**kwargs)
        # fornecer mesmas infos (mas você pode customizar)
